...
module.exports = function(ConsoleLogger){
    ...
    ConsoleLogger.prototype.lineBreak = function (lineString, color) {
        ...
        this.logRaw('trace', lineString, color);
        return this;
    };
    ...
    ConsoleLogger.prototype.serviceCallStart = function (serviceName) {
        ...
        var logLine = util.format('=====>  SERVICE CALL: [%s]  <====', serviceName);
        this.logRaw('trace', logLine, 'yellowBright');
        return serviceCall;
    };
    ...
    ConsoleLogger.prototype.serviceCallEnd = function (serviceCall, response) {
        ...
        var logLines = [];
        logLines.push(util.format('=====>  SERVICE CALL COMPLETED (%d seconds): [%s]  <====', serviceCall.totalTime, serviceCall.serviceName));
        if(response){
            if(response !== null && typeof response === 'object'){
                logLines = logLines.concat(ConsoleLogger.jsonLines(response))
            }
            else {
        ...
        var indent = this._getIndent();
        var lineString = logLines.map(function(line){
            return indent + line;
        }).join('\n');

        this.logRaw('trace', lineString, 'yellowBright');
        return this;
    };
    ...
    ConsoleLogger.prototype.exception = function (exception) {
        ...
        if(this._options.dumpExceptions){
            ...
            //}
            this.logRaw('error', lines.join('\n'));
        }
        return this;
    };


    ConsoleLogger.prototype.logRaw = function (logMessageLevelName, message, colorOverride) {
        var logMessageLevel = this._getLogLevel(logMessageLevelName);
        if(logMessageLevel){
            return this._log(logMessageLevel, message, colorOverride);
        }
        return this;
    };
